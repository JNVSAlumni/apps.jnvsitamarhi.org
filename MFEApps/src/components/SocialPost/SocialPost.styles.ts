...
export const headerStyles = (type: IPostType, dual = false): React.CSSProperties => ({
  ...
  justifyContent: "center",
  gap: dual ? 14 : 16,
  padding: dual ? "11px 22px" : "20px 26px",
  background: `linear-gradient(125deg, ${type.headerFrom} 0%, ${type.headerTo} 100%)`,
  color: "#ffffff",
  boxShadow: "inset 0 -1px 0 rgba(0,0,0,0.08)",
  // Only the dual (BNP) header keeps a fixed height so the taller two-line
  // title is not clipped; the single-logo header is left exactly as before.
  ...(dual ? { flexShrink: 0 } : {}),
});
...
/* ---- Dual-logo header (only used when "Include BNP" is checked) ---- */
